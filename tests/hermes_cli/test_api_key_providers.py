...
class TestProviderRegistry:
    ...
    def test_base_urls(self):
        ...
        assert (
            PROVIDER_REGISTRY["ai-gateway"].inference_base_url
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr(
            ...
            lambda: (
                "tid=session_token",
                "https://api.githubcopilot.com",
                "copilot-session via gh auth token",
            ),
        )
        status = get_api_key_provider_status("copilot")
        assert status["configured"] is True
        assert status["logged_in"] is True
        assert "gh auth token" in status["key_source"]
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_with_github_token(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.copilot_auth.resolve_copilot_token",
            lambda: (
                "tid=session_from_github_token",
                "https://api.githubcopilot.com",
                "copilot-session via GITHUB_TOKEN",
            ),
        )
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
        assert creds["api_key"] == "tid=session_from_github_token"
        assert creds["base_url"] == "https://api.githubcopilot.com"
        assert "GITHUB_TOKEN" in creds["source"]

    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.copilot_auth.resolve_copilot_token",
            lambda: (
                "tid=session_from_cli",
                "https://api.githubcopilot.com",
                "copilot-session via gh auth token",
            ),
        )
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
        assert creds["api_key"] == "tid=session_from_cli"
        assert creds["base_url"] == "https://api.githubcopilot.com"
        assert "gh auth token" in creds["source"]
