...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        # Track pending model picker state per session
        # Key: session_key, Value: {"stage": str, "selected_provider": str, "timestamp": float}
        self._pending_model_picker: Dict[str, Dict[str, Any]] = {}
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        lines.append("Interactive picker: `/models`")
        lines.append("Setup: `hermes setup`")
        return "\n".join(lines)

    async def _handle_models_command(self, event: MessageEvent) -> str:
        """Handle /models command - interactive model picker."""
        ...
        picker_state = self._pending_model_picker.get(session_key)

        if picker_state:
            stage = picker_state.get("stage", "provider")
            timestamp = picker_state.get("timestamp", 0)

            if current_time - timestamp > PICKER_TIMEOUT_SECONDS:
                self._pending_model_picker.pop(session_key, None)
                picker_state = None

        if args == "cancel" or args == "stop":
            self._pending_model_picker.pop(session_key, None)
        ...
        if current_provider == "auto" or not current_provider:
            current_provider = "openrouter"

        if picker_state and stage == "model":
            selected_provider = picker_state.get("selected_provider", "")
            choice_idx = parse_model_choice(
                args, get_model_list(selected_provider, current_model=current_model)
            )

            if choice_idx is None:
                return f"Invalid choice. Reply with a number (1-{len(get_model_list(selected_provider))}) or `/models cancel` to exit."

            models = get_model_list(selected_provider, current_model=current_model)
            selected_model = models[choice_idx]

            self._pending_model_picker.pop(session_key, None)

            result = switch_model(
                raw_input=f"{selected_provider}:{selected_model.id}",
                current_provider=current_provider,
            )

            if result.success:
                favorites = load_favorites()
                favorites[selected_provider] = selected_model.id
                from hermes_cli.model_picker import save_favorites
                ...
                return f"❌ Failed to switch model: {result.error_message}"

        if picker_state and stage == "provider":
            providers = get_provider_list(current_provider)
            choice_idx = parse_model_choice(args, providers)

            if choice_idx is None:
                return f"Invalid choice. Reply with a number (1-{len(providers)}) or `/models cancel` to exit."

            selected_provider = providers[choice_idx].id

            favorites = load_favorites()
            models = get_model_list(selected_provider, favorites, current_model)

            if not models:
                return f"❌ No models available for provider `{selected_provider}`."

            self._pending_model_picker[session_key] = {
                "stage": "model",
                "selected_provider": selected_provider,
            ...
            return format_model_selection(selected_provider, provider_label, models)
        ...
        return format_provider_selection(providers)
