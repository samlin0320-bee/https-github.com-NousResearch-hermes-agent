...
if [[ "$EVAL_SOURCE" == "sessiondb" ]]; then
  ...
  if [[ -z "$BRIDGE_PYTHON" && -x "$ROOT_DIR/venv/bin/python" ]]; then
    BRIDGE_PYTHON="$ROOT_DIR/venv/bin/python"
  fi
  if [[ -z "$BRIDGE_PYTHON" && -x "/usr/bin/python3" ]]; then
    BRIDGE_PYTHON="/usr/bin/python3"
  fi
  if [[ -z "$BRIDGE_PYTHON" && -x "/usr/local/bin/python3" ]]; then
    BRIDGE_PYTHON="/usr/local/bin/python3"
  fi
  if [[ -z "$BRIDGE_PYTHON" ]]; then
    BRIDGE_PYTHON="$(command -v python3 2>/dev/null || true)"
  fi
  if [[ -z "$BRIDGE_PYTHON" ]]; then
    BRIDGE_PYTHON="$(command -v python 2>/dev/null || true)"
  fi
  if [[ -z "$BRIDGE_PYTHON" ]]; then
    echo "[self-evo] unable to find python interpreter for state.db export" >&2
    exit 127
  fi
  echo "[self-evo] exporting state.db sessions -> $TEMP_HOME/.hermes/sessions"
  "$BRIDGE_PYTHON" -m agent.self_evolution_bridge \
