...
def _common_betas_for_base_url(base_url: str | None) -> list[str]:
    ...
    MiniMax's Anthropic-compatible endpoints accept Bearer auth but have been
    observed to fail tool-use requests when Hermes forces Anthropic's
    fine-grained tool streaming beta. Keep the shared thinking beta there, but
    omit tool-streaming so tool calls fall back to the provider's default path.
    """
    if _requires_bearer_auth(base_url):
        return _MINIMAX_COMPAT_BETAS
