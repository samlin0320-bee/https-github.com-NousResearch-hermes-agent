...
def _warn_invalid_context_length(scope: str, model: str, raw_value: Any) -> None:
    """Emit runtime warnings for invalid context-length config values."""
    if scope == "custom_providers":
        logger.warning(
...
class AIAgent:
    ...
    def __init__(
        ...
        _config_context_length = resolve_config_context_length(
            ...
            warn=_warn_invalid_context_length,
        )
    ...
    def switch_model(
        ...
        is_native_anthropic = (
            api_mode == "anthropic_messages" and new_provider == "anthropic"
        )
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            _previous_config_context_length = getattr(
                self, "_config_context_length", None
            )
            try:
                from hermes_cli.config import load_config as _load_config_for_switch

                _switch_cfg = _load_config_for_switch()
                self._config_context_length = resolve_config_context_length(
                    model=self.model,
                    provider=self.provider,
                    base_url=self.base_url,
                    agent_cfg=_switch_cfg,
                    warn=_warn_invalid_context_length,
                )
            except Exception:
    ...
    def _check_compression_model_feasibility(self) -> None:
        ...
        try:
            ...
            if aux_context < threshold:
                ...
                safe_pct = int(
                    (aux_context / self.context_compressor.context_length) * 100
                )
                msg = (
    ...
    def _try_activate_fallback(self) -> bool:
        ...
        try:
            ...
            is_native_anthropic = (
                fb_api_mode == "anthropic_messages" and fb_provider == "anthropic"
            )
            self._use_prompt_caching = (
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "codex_responses":
            ...
            is_xai_responses = (
                self.provider == "xai" or "api.x.ai" in (self.base_url or "").lower()
            )
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if _prior_was_tool and not getattr(
                            self, "_post_tool_empty_retried", False
                        ):
