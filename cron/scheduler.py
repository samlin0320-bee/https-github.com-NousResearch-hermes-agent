...
def _deliver_job_result(
    ...
    *,
    ...
    Delivery happens only after the run has been durably finalized. Delivery
    failures are recorded separately when the finalized run still matches the
    provided ``run_at``.
    """
...
def _dispatch_claimed_jobs(claimed_jobs: List[dict], max_parallel_jobs: int, verbose: bool, adapters=None, loop=None) -> int:
    if not claimed_jobs:
        return 0
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    submitted = _dispatch_claimed_jobs(claimed_jobs, max_parallel_jobs, verbose, adapters=adapters, loop=loop)
    if verbose:
