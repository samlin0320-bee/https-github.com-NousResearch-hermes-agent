...
The web tools support five backend providers, configured via `config.yaml` or `hermes tools`:
...
If `web.backend` is not set, the backend is auto-detected from available API keys (priority: Firecrawl → Parallel → Tavily → Kimi → Exa). Self-hosted Firecrawl is also supported via `FIRECRAWL_API_URL`.
