...
| `VENICE_API_KEY` | Venice API key ([venice.ai/settings/api](https://venice.ai/settings/api)) |
| `VENICE_BASE_URL` | Override Venice base URL (default: `https://api.venice.ai/api/v1`) |
...
| Variable | Description |
|----------|-------------|
| `HERMES_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `kilocode`, `xiaomi`, `arcee`, `alibaba`, `deepseek`, `opencode-zen`, `opencode-go`, `ai-gateway`, `venice` (default: `auto`) |
