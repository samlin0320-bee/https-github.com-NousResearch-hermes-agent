...
description: "External memory provider plugins — Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, Supermemory, PowerMem"
---
...
```yaml
memory:
  provider: openviking   # or honcho, mem0, hindsight, holographic, retaindb, byterover, supermemory, powermem
```
...
Hybrid vector + full-text (+ optional graph) memory with LLM-based extraction, deduplication, and Ebbinghaus-style retrieval. Runs **inside the Hermes process** via the PowerMem Python SDK — no separate server required if your `.env` configures a local vector store.

| | |
|---|---|
| **Best for** | Self-hosted or cloud-backed memory with intelligent turn ingestion and hybrid search |
| **Requires** | `pip install powermem` (see optional extra `hermes-agent[powermem]`) + PowerMem `.env` (vector store + embedder; LLM recommended for intelligent `add` on each turn) |
| **Data storage** | Your configured backend (SQLite, PostgreSQL, OceanBase / SeekDB, etc.) |
| **Cost** | Depends on your LLM/embedding APIs and database (infra can be free locally) |

**Tools:** `powermem_profile` (list recent memories), `powermem_search` (hybrid / semantic search), `powermem_add` (verbatim store, `infer=false` — aligned with PowerMem `Memory.add`)
...
# Merge PowerMem variables into $HERMES_HOME/.env (plugin sets POWERMEM_ENV_FILE to that path)
```

**Config:** `$HERMES_HOME/powermem.json` (optional Hermes-side `user_id`, `agent_id`, `enabled`) · Full PowerMem key reference: [PowerMem configuration guide](https://github.com/oceanbase/powermem/blob/main/docs/guides/0003-configuration.md)

**Key features:**
- Automatic `POWERMEM_ENV_FILE` wiring to the active Hermes profile `.env`
- Per-turn background sync with intelligent extraction (`infer=true`)
- Mirrors built-in memory tool writes to PowerMem (`infer=false`)

**Support:** [PowerMem on GitHub](https://github.com/oceanbase/powermem)
...
| **PowerMem** | Self-hosted / Cloud | Varies | 3 | `powermem` | Hybrid retrieval + SDK-native `add` / search |
...
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory, PowerMem) store config in `$HERMES_HOME/` so each profile has its own credentials
