...
class ProcessRegistry:
    ...
    def get_for_task(self, session_id: str, task_id: str = "") -> Optional[ProcessSession]:
        """Get a session by ID, but only if it belongs to the calling task.

        Returns None when the session does not exist or belongs to a different
        task. This avoids leaking the existence of background processes across
...
def _handle_process(args, **kw):
    ...
    if action == "list":
        ...
        if action == "poll":
            return _json.dumps(process_registry.poll(session_id, task_id=task_id or ""), ensure_ascii=False)
        elif action == "log":
            ...
            return _json.dumps(process_registry.wait(session_id, timeout=args.get("timeout"), task_id=task_id or ""), ensure_ascii=False)
        elif action == "kill":
            return _json.dumps(process_registry.kill_process(session_id, task_id=task_id or ""), ensure_ascii=False)
        elif action == "write":
            return _json.dumps(process_registry.write_stdin(session_id, str(args.get("data", "")), task_id=task_id or ""), ensure_ascii=False)
        elif action == "submit":
            return _json.dumps(process_registry.submit_stdin(session_id, str(args.get("data", "")), task_id=task_id or ""), ensure_ascii=False)
        elif action == "close":
            return _json.dumps(process_registry.close_stdin(session_id, task_id=task_id or ""), ensure_ascii=False)
