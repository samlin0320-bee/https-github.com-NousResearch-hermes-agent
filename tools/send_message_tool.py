...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None):
    ...
    _MEDIA_CAPABLE = {Platform.TELEGRAM, Platform.DISCORD, Platform.FEISHU, Platform.WEIXIN}
    if media_files and not message.strip() and platform not in _MEDIA_CAPABLE:
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for telegram, discord, feishu, and weixin; "
    ...
    if media_files and platform not in _MEDIA_CAPABLE:
        warning = (
            ...
            "native send_message media delivery is currently only supported for telegram, discord, feishu, and weixin"
        )
