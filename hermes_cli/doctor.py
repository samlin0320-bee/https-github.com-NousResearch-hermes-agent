...
def check_fail(text: str, detail: str = ""):
    print(f"  {color('[X]', Colors.RED)} {text}" + (f" {color(detail, Colors.DIM)}" if detail else ""))
...
def run_doctor(args):
    ...
    print(color("|                 [MEDICAL] Hermes Doctor                        |", Colors.CYAN))
    print(color("+---------------------------------------------------------+", Colors.CYAN))
    ...
    if openrouter_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"\r  {color('[X]', Colors.RED)} OpenRouter API {color('(invalid API key)', Colors.DIM)}                ")
                issues.append("Check OPENROUTER_API_KEY in .env")
            else:
                print(f"\r  {color('[X]', Colors.RED)} OpenRouter API {color(f'(HTTP {response.status_code})', Colors.DIM)}                ")
        except Exception as e:
            print(f"\r  {color('[X]', Colors.RED)} OpenRouter API {color(f'({e})', Colors.DIM)}                ")
    ...
    if anthropic_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"\r  {color('[X]', Colors.RED)} Anthropic API {color('(invalid API key)', Colors.DIM)}                 ")
            else:
                msg = "(couldn't verify)"
        ...
        if _key:
            ...
            try:
                ...
                if _resp.status_code == 200:
                    ...
                    print(f"\r  {color('[X]', Colors.RED)} {_label} {color('(invalid API key)', Colors.DIM)}           ")
    ...
    try:
        ...
        if has_aws_credentials():
            ...
            try:
                ...
                print(f"\r  {color('[WARN]', Colors.YELLOW)} {_label} {color(f'(boto3 not installed — {sys.executable} -m pip install boto3)', Colors.DIM)}           ")
