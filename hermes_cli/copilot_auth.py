...
Token type support (per GitHub docs):
  ...
  ghp_          Classic PAT           [X]  NOT SUPPORTED
...
def copilot_device_code_login(
    *,
    ...
    try:
        ...
        print(f"  [X] Failed to start device authorization: {exc}")
        return None
    ...
    if not device_code or not user_code:
        print("  [X] GitHub did not return a device code.")
        return None
    ...
    while time.time() < deadline:
        ...
        if error == "authorization_pending":
            ...
            print()
            print("  [X] Device code expired. Please try again.")
            return None
        elif error == "access_denied":
            print()
            print("  [X] Authorization was denied.")
            return None
        elif error:
            print()
            print(f"  [X] Authorization failed: {error}")
            return None
